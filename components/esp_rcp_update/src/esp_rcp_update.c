...
esp_err_t esp_rcp_update(void)
{
    ...
    ret = loader_port_esp32_init(&loader_config);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to initialize UART port: %s", esp_err_to_name(ret));
        return ret;
    }
    loader_inited = true;

    esp_loader_error_t loader_err = connect_to_target(s_handle.update_config.target_chip,
                                                      s_handle.update_config.update_baudrate);
    if (loader_err != ESP_LOADER_SUCCESS) {
        ESP_LOGE(TAG, "Failed to connect to RCP bootloader: %d", loader_err);
        ret = ESP_FAIL;
        goto cleanup;
    }
    ...
    fp = fopen(fullpath, "r");
    if (fp == NULL) {
        ESP_LOGE(TAG, "Cannot find rcp image: %s", fullpath);
        ret = ESP_ERR_NOT_FOUND;
        goto cleanup;
    }
    esp_rcp_subfile_info_t subfile;
    ret = seek_to_subfile(fp, FILETAG_RCP_FLASH_ARGS, &subfile);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to find RCP flash arguments in %s", fullpath);
        goto cleanup;
    }
    ...
    for (int i = 0; i < num_flash_binaries; i++) {
        ...
        if (fread(&flash_args, 1, sizeof(flash_args), fp) != sizeof(flash_args)) {
            ESP_LOGE(TAG, "Failed to read RCP flash arguments from %s", fullpath);
            ret = ESP_FAIL;
            goto cleanup;
        }
        int num_retry = 0;
        ...
        while (flash_binary(fp, subfile.size, flash_args.offset) != ESP_LOADER_SUCCESS) {
            ...
            if (num_retry > RCP_UPDATE_MAX_RETRY) {
                ESP_LOGE(TAG, "Failed to update RCP after %d retries", RCP_UPDATE_MAX_RETRY);
                ret = ESP_FAIL;
                goto cleanup;
            }
            if (seek_to_subfile(fp, flash_args.tag, &subfile) != ESP_OK) {
                ESP_LOGE(TAG, "Failed to seek to subfile with tag %lu before retry", flash_args.tag);
                ret = ESP_FAIL;
                goto cleanup;
            }
        }
        if (fseek(fp, current, SEEK_SET) != 0) {
            ESP_LOGE(TAG, "Failed to restore RCP image file offset");
            ret = ESP_FAIL;
            goto cleanup;
        }
    }
...
#if CONFIG_OPENTHREAD_RADIO_SPINEL_SPI
    ret = esp_rcp_boot_pin_mux();
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to multiplex boot pin: %s", esp_err_to_name(ret));
    }
#endif
